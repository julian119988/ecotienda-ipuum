...
export const checkPass = async (req, res, next) => {
  ...
  try {
    ...
    if (savedUser) {
      bcrypt.compare(data.password, savedUser.password, function (err, result) {
        if (result) {
          res.send({
...
export const getProductos = async (req, res, next) => {
  ...
  try {
    if (busqueda === "all") {
      ...
      const response = await ProductosModel.find({
        $or: [{ producto: { $regex: regex } }, { marca: { $regex: regex } }],
      }).exec();
      res.send(response);
    }
  } catch (err) {
    ...
    if (productoAntiguo.campo) {
      if (productoAntiguo.campo === "cantidad") {
        const newHistory = new HistorialModel({
          ...
          cambioStock: productoAntiguo.campo === "cantidad" ? true : false,
...
export const getFraccionamiento = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(413)
      .send("Ha ocurrido un error al buscar los fraccionamientos.");
  }
};
export const postFraccionamiento = async (req, res, next) => {
  ...
  try {
    ...
    res.status(413).send("Ha ocurrido un error guardando el fraccionamiento.");
    next(err);
  }
};
